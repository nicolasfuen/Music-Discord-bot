...
module.exports = {
  name: "search",
  description: "Muestra un resultado de canciones según la consulta de búsqueda.",
  usage: "[song]",
  permissions: {
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Tardaste demasiado en responder. ¡Ejecute el comando nuevamente si quiere jugar algo!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Envíe el número de identificación de la canción correcto"
        );
      ...
      SongAddedEmbed.setAuthor(`Agregado a la cola`, client.botconfig.IconURL);
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Ingrese el nombre de la canción o la URL que desea buscar",
      },
    ],
    /**
     *
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "NO_MATCHES":
            ...
            return client.sendTime(
              interaction,
              ":x: | **No se encontraron resultados**"
            );
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Agregado a la cola:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
        ...
        switch (res.loadType) {
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Agregado a la cola:** \`[${res.tracks[0].title}](${res.tracks[0].uri})\`.`
            );
          case "PLAYLIST_LOADED":
          ...
          case "SEARCH_RESULT":
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `El número que proporcionó fue mayor o menor que el total de búsqueda. Uso - \`(1-${max})\``
              );
