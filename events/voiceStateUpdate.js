    ...
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          .setAuthor(`Reanudando la cola en pausa`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(
            `Reanudando la reproducción porque todos ustedes me dejaron con música para tocar solo`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          ...
          .setColor(client.botconfig.EmbedColor)
          .setDescription(`El reproductor se detuvo porque todos se fueron`);
        await client.channels.cache.get(player.textChannel).send(emb);
      }
      break;
  }
