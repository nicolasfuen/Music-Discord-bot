...
class DiscordMusicBot extends Client {
  constructor(props) {
    ...
    if (this.botconfig.Token === "")
      return new TypeError(
        "El botconfig.js no está completo. Asegúrese de que no haya nada en blanco, de lo contrario, el bot no funcionará correctamente."
      );
    ...
    this.Manager = new Manager({
      ...
      .on("nodeConnect", (node) =>
        this.log(`Lavalink: Node ${node.options.identifier} conectado`)
      )
      .on("nodeError", (node, error) =>
        this.log(
          `Lavalink: Node ${node.options.identifier} tuvo un error: ${error.message}`
        )
      )
      .on("trackStart", async (player, track) => {
        this.SongsPlayed++;
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Reproduciendo ♪`, this.botconfig.IconURL)
          .setThumbnail(player.queue.current.displayThumbnail())
          .setDescription(`[${track.title}](${track.uri})`)
          .addField("Solicitado por", `${track.requester}`, true)
          .addField(
            "Duracion",
      ...
      .on("queueEnd", (player) => {
        let QueueEmbed = new MessageEmbed()
          .setAuthor("La cola ha terminado", this.botconfig.IconURL)
          .setColor(this.botconfig.EmbedColor)
          .setTimestamp();
          ...
          if (!cmd.name || !cmd.description || !cmd.run)
            return this.log(
              "No se puede cargar el comando: " +
                file.split(".")[0] +
                ", Reason: El archivo no tenía run/name/desciption"
            );
          this.commands.set(file.split(".")[0].toLowerCase(), cmd);
          this.log("Comando cargado: " + file.split(".")[0]);
        });
    });
  }
    ...
    fs.readdir(EventsDir, (err, files) => {
      if (err) this.log(err);
        ...
        files.forEach((file) => {
          ...
          this.logger.log("Evento cargado: " + file.split(".")[0]);
        });
    });
  }
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Si cree que esto es un error, infórmelo en el servidor de soporte!"
      );
  ...
  build() {
    ...
    if (this.botconfig.ExpressServer) {
      this.http.listen(process.env.PORT || this.botconfig.Port, () =>
        this.log("Se ha iniciado el servidor web")
      );
    }
  }
